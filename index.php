...
<!-- Page Content -->
<br><br><h2> Recent Uploads </h2>
	<?php 
		$result = mysqli_query($con, "SELECT * FROM lectures WHERE module_id IN (SELECT module_id FROM usermodules WHERE user_id='".getUserId($_SESSION['username'])."') ORDER BY upload_time LIMIT 12");
		$output = '<div class="scrolling-video-flexbox" style="display: flex;flex-wrap: nowrap;overflow-x: auto;width: 85%;">';
		$counter = 0;
		while($row = $result->fetch_assoc()) {
			if($counter < 6) {
				// To switch back simply uncomment the button and switch the divs class back to 'video'
				$output .= '<div class="watchLecture" id="'.$row['video_id'].'" style="flex: 0 0 auto;">';
				$output .= '<figure style=width:300px;>';
				$output .= '<img src="https://img.youtube.com/vi/'.$row['video_id'].'/sddefault.jpg" width=95%>'; // width=50% height=70% 
				$output .= '<figcaption>'.getYoutubeTitle($row['video_id']).'</figcaption>';
				$output .= '</figure>';
				//$output .= '<button class="watchLecture" id="'.$row['video_id'].'">Watch lecture</button>';
				$output .= '</div>';
			} else {
				if($counter == 6) {
					$output .= '</div>';
				...
				$output .= '<div class="watchLecture" id="'.$row['video_id'].'" style="flex: 0 0 auto;">';
				$output .= '<figure style=width:300px;>';
				$output .= '<img src="https://img.youtube.com/vi/'.$row['video_id'].'/sddefault.jpg" width=95%>';
				$output .= '<figcaption>'.getYoutubeTitle($row['video_id']).'</figcaption>';
				$output .= '</figure>';
				//$output .= '<button class="watchLecture" id="'.$row['video_id'].'">Watch lecture</button>';
				$output .= '</div>';
		...
		if($counter == 0) {
			$output .= "No Recent uploads found";
		}
		$output .= '</div>';
		echo $output;
		function getYoutubeTitle($video_id){
			$html = 'https://www.googleapis.com/youtube/v3/videos?id='.$video_id.'&key='; // ADD API KEY HERE
...
<h2>Modules</h3>
	<?php 
		$result = mysqli_query($con, "SELECT * FROM modules WHERE module_id IN (SELECT module_id FROM usermodules WHERE user_id='".getUserId($_SESSION['username'])."')");
		$output = '<div class="scrolling-modules-flexbox" style="display: flex;flex-wrap: nowrap;overflow-x: auto;width: 85%;">';
		$counter  = 0;
		while($row = $result->fetch_assoc()) {
			$output .= '<div class="module" style="flex: 0 0 auto; height: 210px;width:30%; border-style: solid; overflow: hidden">';
			$output .= '<div><a href="modulepage.php?id='.$row['module_id'].'"><h4>'.$row['module_id'].': '.$row['module_name'].'</a></h4>';
			$output .= $row['module_desc'].'</div>';
			$output .= '</div>';
			$counter++;
		}
		if($counter == 0) {
			$output .= '<a href="modules.php">You are not following any modules! Go follow some!</a>';
		}
		$output .= '</div>';
		echo $output;
	?>
