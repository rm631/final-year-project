...
function syncVideo(roomnum) {
    // added check for if the person trying to force the sync is the host by rm631
	if(host) {
		var currTime = 0
		var state
		var videoId = id
...
socket.on('syncVideoClient', function(data) {
	console.log("syncVideoClient, socketid: " + socket.id);
	console.log("syncVideoClient, hostsocketid: " + host);
