...
server.listen(process.env.PORT || 3000); // 3000
...
con.connect(function(err) {
	...
	setInterval(function() {
		if (err) throw err;
		var sql = "DELETE FROM `invites` WHERE `time_sent` < DATE_SUB(NOW(), INTERVAL 15 MINUTE)";
		con.query(sql, function (err, result) {
			if (err) throw err;
			console.log("No. of invites deleted: " + result.affectedRows);
		});
	}, the_interval);
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('new room', function(data, callback) {
        ...
        if (socket.id != host) {
            ...
            console.log("Non-host connected, calling host socket: " + host)
            ...
            console.log("This is the host socket, no sync required")
