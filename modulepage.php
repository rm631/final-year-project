...
<main>
	<?php 
		$module_id = filter_var($_GET['id'], FILTER_SANITIZE_STRING);
		$result = mysqli_query($con, "SELECT * FROM modules WHERE module_id='$module_id'");
		$output = '';
		while($row = $result->fetch_assoc()) {
			$output .= '<div style="width: 75%">';
			$output .= '<h1 id="module_title">'.$row['module_id'].': '.$row['module_name'].'</h1>';
			$output .= '<p id="module_desc"><h5>'.$row['module_desc'].'</h5></p>';
			$output .= '</div>';
		}
		
		$output .= '<h2>Lectures</h2>';
		$result2 = mysqli_query($con, "SELECT * FROM lectures WHERE module_id='$module_id' ORDER BY upload_time LIMIT 12");
		...
		function getYoutubeTitle($video_id){
			$html = 'https://www.googleapis.com/youtube/v3/videos?id='.$video_id.'&key=AIzaSyAjQQZuN0FxlpQrnw4yoNbWmuM9xh5qX2k&part=snippet';
